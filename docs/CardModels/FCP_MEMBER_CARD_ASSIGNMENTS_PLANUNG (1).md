...
## Systemgrenze: Kartenmodell vs. Abrechnungsmodell

**Das ist eine harte Grenze. Sie darf nicht verwischen.**

| Ebene | Frage | Zuständig |
|---|---|---|
| Kartenmodell | Wo darf jemand angeln? | `member_card_assignments` |
| Abrechnungsmodell | Was muss jemand zahlen? | Späteres Billing-Modul |

### `Btr. bf` / `VSS Btr. Bf` — fachliche Einordnung

Diese Werte tauchen aktuell in `fishing_card_type` auf:
- `Innenwasser + Rheinlos VSS Btr. Bf` (10 Mitglieder)
- `Innenwasser + Rheinlos Btr. bf` (5 Mitglieder)
- `Innenwasser Btr. bf` (1 Mitglied)

**Was das bedeutet:** `Btr. bf` steht für „Beitrag befreit" — das ist kein Kartentyp, sondern ein Abrechnungsstatus. Das Mitglied hat die Berechtigung (Karte), zahlt aber keinen oder einen reduzierten Beitrag.

**Fachregel:**
```
Btr. bf / VSS Btr. Bf sind KEINE Karten und KEINE Berechtigungskontexte.
Sie sind Abrechnungsmarker.

Bei der Migration zu member_card_assignments:
- NICHT als card_id interpretieren
- NICHT ins Kartenmodell übernehmen
- Als separates Feld / Flag im späteren Abrechnungsmodul modellieren

Kartenmodell und Abrechnungsmodell bleiben strukturell getrennt.
```

**Konsequenz für die Migrationsstrategie:**
Bevor `fishing_card_type`-Strings migriert werden, müssen die Beitragsvermerke
herausgefiltert und separat behandelt werden. Das ist eine fachliche Klärung
mit dem Verein — keine technische Entscheidung.

**Betroffene Mitglieder beim VDAN:** 16 Mitglieder haben `Btr. bf`-Marker.
Diese müssen im Migrationsprozess manuell klassifiziert werden.
...
**Achtung — Beitragsvermerke herausfiltern vor Migration:**

| String-Fragment | Ist | Behandlung |
|---|---|---|
| `Btr. bf` | Beitragsbefreiung | → Abrechnungsmodul, NICHT in `card_id` |
| `VSS Btr. Bf` | VSS-Beitragsbefreiung | → Abrechnungsmodul, NICHT in `card_id` |
| `Erwachsener` | Altersgruppe (redundant zu `standard`) | → weglassen, aus `member_group_key` ableiten |
| `Jugend` | Altersgruppe | → `youth` in `member_group_key` |
| `Ehrenmitglied` | Gruppe | → `honorary` in `member_group_key` |

Betroffene VDAN-Mitglieder mit Beitragsvermerk: **16**
→ Diese 16 brauchen im Migrationsprozess manuelle Klärung mit dem Verein.
