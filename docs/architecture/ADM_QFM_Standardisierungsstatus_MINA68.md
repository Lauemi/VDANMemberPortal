...
**Nicht übernehmbar ohne eigene Arbeit:** 2-Spalten-Layout für QFM-Formulare (`.qfp-shell`). Erfordert separate CSS-Regeln unter `.qfp-shell .qfp-form-grid` — fehlt bisher komplett.
...
|---|---|---|---|---|---|
| ADM Vereinsverwaltung | `ADM_clubSettings.json` | `/app/mitgliederverwaltung/` | 8× `inline-data-table` | **1× `data-table`** (Ausweise) | Nein — alle `viewMode: "table"` |
...
| **`club_settings_cards_table`** | **Ausweise** | **`data-table` ⚠️** |
...
### P1 — Sofort, minimales Risiko

**`club_settings_cards_table` (Ausweise) → `inline-data-table`**
- Einziger verbleibender `data-table`-Rest in `ADM_clubSettings.json`
- Umstellung analog zu Freigaben-Panel (MINA-68 bereits gezeigt wie)
- Voraussetzung: Prüfen, ob `columns`-Definitionen des Panels für `fcp-inline-data-table-v2.js` kompatibel sind
- Datei: `docs/masks/templates/Onboarding/ADM_clubSettings.json:3260`
- Änderung: `"componentType": "data-table"` → `"componentType": "inline-data-table"`

### P2 — Nächste Iteration, moderat
...
- Problem: CSS unter `.fcp-adm-shell .qfp-form-grid` gilt nicht für `.qfp-shell`-Seiten (einstellungen)
- Lösung: Neue CSS-Regeln in `redesign.css` unter `.qfp-shell .qfp-form-grid` ergänzen
- Scope: Nur einstellungen-Seite (`/app/einstellungen/`) lädt `redesign.css` — minimales Blast-Radius
- Risiko: `ofmMask.css` definiert dort eigene Form-Styles — muss auf Konflikte geprüft werden

**`cards-view` für ausgewählte ADM-Panels aktivieren**
...
- Derzeit: Alle Panels nutzen `viewMode: "table"` — keine cards-view aktiv
- Sinnvoll für: Mitglieder-Registry, Fangfreigaben (kompakte Karten-Ansicht)
- Voraussetzung: Card-Column-Mapping (`cardConfig`) im Panel-JSON definieren

### P3 — Backlog, erst evaluieren

**QFM Onboarding / Anmeldung-Masken**
- `QFM_onboarding.template.json`, `QFM_clubEntryBillingSignIn.json`, `QFM_clubRegister.json` noch nicht gesurveyed
- Laden `redesign.css` ggf. nicht — Rollout setzt Stylesheet-Einbindung voraus
- Erst prüfen ob diese Seiten überhaupt `redesign.css` laden bevor CSS-Standardisierung angewendet wird
...
| **`cards-view` in ADM_clubSettings** | Technisch möglich, aber kein `cardConfig` in den Panel-JSONs definiert. Erst Daten-Mapping klären, dann CSS bereits vorhanden. |
...
|---|---|---|---|---|
| T1 | Ausweise-Panel auf `inline-data-table` umstellen | Code | Keiner | XS — 1 JSON-Zeile + Smoke-Test |
| T2 | QFM-Einstellungen 2-Spalten: CSS-Scope für `.qfp-shell` ergänzen | Code | Conflict-Check `ofmMask.css` vs `redesign.css` | S |
| T3 | Survey: Laden QFM Onboarding-Masken `redesign.css`? | Analyse | Keiner | XS |
| T4 | Cards-View für Mitglieder-Registry aktivieren: `cardConfig` definieren + `viewMode: "both"` | Code | Karten-Spalten-Mapping | M |
