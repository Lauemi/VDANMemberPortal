...
;(() => {
  ...
  function buildTableRuntimeOptions(panel, columns, rows, runtimeContext = {}) {
    ...
    // Domain-Logik wird vom VdanDomainAdapterVereinsverwaltung geliefert.
    // Der Contract Hub bleibt neutral und kennt keine Bereichsimplementierungen.
    const _domainAdapter = window.VdanDomainAdapterVereinsverwaltung || null;
    const _domainHandlers = _domainAdapter
      ? _domainAdapter.createHandlers({ rows, pattern, section, panelId, message })
      : {};
    const createMemberRegistryRow = typeof _domainHandlers.createMemberRegistryRow === "function"
      ? _domainHandlers.createMemberRegistryRow
      : async () => { message("Domain-Adapter nicht geladen."); return false; };
    const updateMemberRegistryRow = typeof _domainHandlers.updateMemberRegistryRow === "function"
      ? _domainHandlers.updateMemberRegistryRow
      : async () => { message("Domain-Adapter nicht geladen."); return false; };
