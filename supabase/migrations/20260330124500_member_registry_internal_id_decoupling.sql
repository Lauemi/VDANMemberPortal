...
begin
  loop
    v_candidate := 'MID-' || upper(substr(replace(gen_random_uuid()::text, '-', ''), 1, 12));

    exit when not exists (
      select 1
      from public.club_members cm
      where cm.member_no = v_candidate
    )
    and not exists (
      select 1
      from public.members m
      where m.membership_number = v_candidate
    );
  end loop;
...
begin
  ...
  select coalesce(
    max(
      case
        when nullif(trim(cm.club_member_no), '') ~ '^[0-9]+$'
          then trim(cm.club_member_no)::int
        when nullif(trim(cm.club_member_no), '') ~ '^[A-Z0-9]{4}-([0-9]+)$'
          then (regexp_match(trim(cm.club_member_no), '^[A-Z0-9]{4}-([0-9]+)$'))[1]::int
        when nullif(trim(cm.member_no), '') ~ '^[A-Z0-9]{4}-([0-9]+)$'
          then (regexp_match(trim(cm.member_no), '^[A-Z0-9]{4}-([0-9]+)$'))[1]::int
        else null
      end
    ),
    0
...
begin
  ...
  if v_club_member_no = '' then
    v_club_member_no := coalesce(
      nullif(trim(v_member.club_member_no), ''),
      public.next_club_member_no(v_member.club_id)
    );
  end if;
