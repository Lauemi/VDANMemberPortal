-- member_invite_claim: canonical_membership_id gegen canonical_memberships absichern
-- (gueltig -> setzen, sonst NULL; Spalte nullable). FK club_user_roles + profiles.
...
declare
  ...
  v_has_prof boolean; v_card_id text; v_card_key text; v_canon uuid;
begin
  ...
  select cm2.id into v_canon from public.canonical_memberships cm2 where cm2.id = v_inv.club_member_id;
  ...
  select v_uid, a.club_id, a.role_key, v_canon, 'invite_claim', 1
  ...
  if not v_has_prof then
    ...
    values
      (v_uid, v_inv.club_id, v_inv.club_id, v_canon, v_mem.member_no,
       v_mem.first_name, v_mem.last_name, v_mem.email, v_card_id, v_card_key);
  else
    update public.profiles p set
      club_id = coalesce(p.club_id, v_inv.club_id),
      tenant_id = coalesce(p.tenant_id, v_inv.club_id),
      canonical_membership_id = coalesce(p.canonical_membership_id, v_canon),
